...
impl ClusterSet {
    ...
    fn spacing(&self) -> Option<Edge> {
        self.cluster_crossing().pop()
    }
    fn cluster_crossing(&self) -> BinaryHeap<Edge>{
...
trait GraphClustering {
    fn split_in_clusters(&self, k: usize) -> Option<ClusterSet>;
}

impl GraphClustering for Graph {
...
mod test {
    ...
    fn test_clustering() {
        ...
        for (filename, result) in test_data {
            println!("{filename}");
            let mut g = Graph::new();
            let Some(cset) = g.load_file_mst(filename).split_in_clusters(4) else { panic!("Returned None instead of a ClusterSet") };
            let Edge(_,NC(_,distance)) = cset.spacing().unwrap() else { panic!("NodeType::N used instead of NodeType::NC") };
            println!("Clusters: {:?}\nDistance: {:?}", cset.clusters, distance);
